  ...
  <div class="home-page">
    ...
    <div class="col-lg-6 mx-auto zones mb">
      <!--zone-editable
        ...
        class="zone"
      /-->
      <zone-editable  v-for="(zone, index) in zones" :key="zone.id" :zone = "zone" class="zone" @edit="update_zone(index,$event.zone)"/>
    </div>
...
export default {
  ...
  methods:{
    update_zone(index,zone_new){
      //se actualiza lista
      this.zones[index].name = zone_new.name
      this.zones[index].updated_at = zone_new.updated_at
      this.zones[index].distributions = zone_new.distributions
    }
  }
